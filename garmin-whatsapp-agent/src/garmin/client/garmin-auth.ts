...
const DEFAULT_TOKEN_DIR = process.env.GARMIN_TOKEN_DIR ?? path.join(os.homedir(), '.garmin-mcp');
...
export class GarminAuth {
  ...
  constructor(email: string, password: string, options?: GarminAuthOptions) {
    this.email = email;
    this.password = password;
    this.promptMfa = options?.promptMfa;
    this.tokenDir = options?.tokenDir ?? DEFAULT_TOKEN_DIR;
    this.loadTokens();
  }
  ...
  private async getLoginTicket(): Promise<string> {
    ...
    if (title.includes('MFA')) {
      if (!this.promptMfa) {
        throw new Error(
          'MFA is required but no MFA handler is available. Re-run the Garmin setup step from the web wizard.',
        );
      }
